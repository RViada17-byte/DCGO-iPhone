    ...
    public class BT20_029 : CEntity_Effect
    {
        public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
        {
            ...
            if (timing == EffectTiming.None)
            {
                bool PermanentCondition(Permanent targetPermanent)
                {
                    return targetPermanent.TopCard.EqualsCardName("Bibimon") ||
                           (targetPermanent.TopCard.IsLevel2 && targetPermanent.TopCard.HasSeekersTraits);
                }
            ...
            if (timing == EffectTiming.BeforePayCost)
            {
                ...
                activateClass.SetHashString("DigivoltuionCost-1_BT20_029");
                cardEffects.Add(activateClass);

                string EffectDiscription()
                {
                    return "[Your Turn] When this Digimon would digivolve into a Digimon card with [Pulsemon] in its text or the [SEEKERS] trait, reduce the digivolution cost by 1.";
                }
                ...
                bool CardCondition(CardSource cardSource)
                {
                    ...
                    {
                        return cardSource.HasSeekersTraits ||
                               cardSource.HasText("Pulsemon");
                    }
            ...
            if (timing == EffectTiming.OnEndBattle)
            {
                ...
                activateClass.SetUpActivateClass(CanActivateCondition, ActivateCoroutine, 1, false, EffectDiscription());
                activateClass.SetHashString("Gain1_BT20-029");
