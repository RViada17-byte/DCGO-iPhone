...
using System;
using Photon.Pun;
public class BT2_111 : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        if (timing == EffectTiming.None)
        {
            bool Condition()
            {
                return card.Owner.TrashCards.Count >= 10 && card.Owner.HandCards.Contains(card);
            }
            ...
            bool PermanentCondition(Permanent targetPermanent)
            {
                ...
                {
                    if (targetPermanent.TopCard.CardNames.Contains("Impmon"))
                    {
                        return true;
                    }
                }
