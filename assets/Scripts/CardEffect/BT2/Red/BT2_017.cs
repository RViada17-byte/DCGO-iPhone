...
public class BT2_017 : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        if (timing == EffectTiming.OnEnterFieldAnyone)
        {
            ...
            {
                return "[When Digivolving] If you have a red Tamer in playÅCdelete 1 of your opponent's Digimon with 3000 DP or less.";
            }
        ...
        if (timing == EffectTiming.None)
        {
            bool Condition()
            {
                if (CardEffectCommons.IsExistOnBattleArea(card))
                {
                    ...
                    {
                        if (card.Owner.Enemy.TrashCards.Count >= 5)
                        {
                            return true;
                        }
                    }
