...
{
    public class EX3_043 : CEntity_Effect
    {
        public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
        {
            ...
            if (timing == EffectTiming.BeforePayCost)
            {
                ...
                activateClass.SetHashString("Digisorption-3_EX3_043");
            ...
            if (timing == EffectTiming.OnEnterFieldAnyone)
            {
                ...
                string EffectDiscription()
                {
                    return "[When Digivolving] If you have 2 or more suspended Digimon in play, unsuspend this Digimon.";
                }
                ...
                bool CanActivateCondition(Hashtable hashtable)
                {
                    if (CardEffectCommons.IsExistOnBattleArea(card))
                    {
                        ...
                        {
                            if (card.Owner.GetBattleAreaDigimons().Count((permanent) => permanent.IsSuspended) >= 2)
                            {
                                return true;
                            }
                        }
