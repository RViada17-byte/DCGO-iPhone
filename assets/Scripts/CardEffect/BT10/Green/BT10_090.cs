...
{
    public class BT10_090 : CEntity_Effect
    {
        public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
        {
            ...
            if (timing == EffectTiming.OnEnterFieldAnyone)
            {
                ...
                activateClass.SetUpICardEffect("Play 1 [Ballistamon] from hand or under your Tamer", CanUseCondition, card);
                activateClass.SetUpActivateClass(CanActivateCondition, ActivateCoroutine, -1, true, EffectDiscription());
                cardEffects.Add(activateClass);

                string EffectDiscription()
                {
                    return "[On Play] You may play 1 [Ballistamon] from your hand or from under one of your Tamers without paying its memory cost.";
                }

                bool CanSelectCardCondition(CardSource cardSource)
                {
                    if (cardSource.CardNames.Contains("Ballistamon"))
                    {
                    ...
                    if (card.Owner.GetBattleAreaPermanents().Count(CanSelectPermanentCondition) >= 1 || card.Owner.HandCards.Count(CanSelectCardCondition) >= 1)
                    {
                        if (card.Owner.GetBattleAreaPermanents().Count(CanSelectPermanentCondition) >= 1 && card.Owner.HandCards.Count(CanSelectCardCondition) >= 1)
                        {
                            if (card.Owner.isYou)
                            {
                                ...
                                {
                                    new Command_SelectCommand($"Under Tamer", () => photonView.RPC("SetFromHand", RpcTarget.All, false), 0),
            ...
            if (timing == EffectTiming.OnEnterFieldAnyone)
            {
                ...
                string EffectDiscription()
                {
                    return "[Your Turn] When you play a Digimon with [Xros Heart] in its traits, by suspending this Tamer, gain 1 memory.";
                }
