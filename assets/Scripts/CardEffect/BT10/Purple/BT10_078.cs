...
{
    public class BT10_078 : CEntity_Effect
    {
        public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
        {
            ...
            if (timing == EffectTiming.OnDestroyedAnyone)
            {
                bool Condition()
                {
                    ...
                    {
                        if (card.PermanentOfThisCard().DigivolutionCards.Count((cardSource) => cardSource.ContainsCardName("Gammamon")) >= 1)
                        {
                            return true;
                        }
                    }
            ...
            if (timing == EffectTiming.OnDestroyedAnyone)
            {
                ...
                {
                    return "[On Deletion] You may play 1 [Gammamon] from your trash suspended without paying its memory cost.";
                }
