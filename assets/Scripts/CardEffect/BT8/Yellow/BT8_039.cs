...
using System;
using Photon.Pun;
public class BT8_039 : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        if (timing == EffectTiming.None)
        {
            ...
            {
                return targetPermanent.TopCard.CardNames.Contains("Terriermon");
            }
        ...
        if (timing == EffectTiming.OnEnterFieldAnyone)
        {
            ...
            {
                return "[When Digivolving] Suspend 1 of your opponent's Digimon for each Tamer you have in play. Then, up to 3 of your opponent's suspended Digimon get -5000 DP for the turn.";
            }
            ...
            IEnumerator ActivateCoroutine(Hashtable _hashtable)
            {
                if (CardEffectCommons.HasMatchConditionPermanent(CanSelectPermanentCondition))
                {
                    int maxCount = Math.Min(card.Owner.GetBattleAreaPermanents().Count((permanent) => permanent.IsTamer), CardEffectCommons.MatchConditionPermanentCount(CanSelectPermanentCondition));
