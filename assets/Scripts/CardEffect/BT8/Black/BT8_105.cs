...
using System;
using Photon.Pun;
public class BT8_105 : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        if (timing == EffectTiming.OptionSkill)
        {
            ...
            {
                return "[Main] Choose any number of your opponent's Digimon whose play costs add up to 15 or less and delete them.";
            }
        ...
        {
            ActivateClass activateClass = new ActivateClass();
            activateClass.SetUpICardEffect($"Delete 1 Digimon with 15 or less Cost", CanUseCondition, card);
            activateClass.SetUpActivateClass(null, ActivateCoroutine, -1, false, EffectDiscription());
            ...
            string EffectDiscription()
            {
                return "[Security] Delete 1 of your opponent's Digimon with a play cost of 15 or less.";
            }
            ...
            bool CanSelectPermanentCondition(Permanent permanent)
            {
                if (CardEffectCommons.IsPermanentExistsOnOpponentBattleAreaDigimon(permanent, card))
                {
                    if (permanent.TopCard.GetCostItself <= 15)
                    {
                        if (permanent.TopCard.HasPlayCost)
                        {
                            return true;
