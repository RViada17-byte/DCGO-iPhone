...
{
    public class EX1_053 : CEntity_Effect
    {
        public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
        {
            ...
            if (timing == EffectTiming.None)
            {
                int count()
                {
                    return card.Owner.TrashCards.Count((cardSource) => cardSource.IsDigimon && cardSource.ContainsCardName("Etemon")); ;
                }
            ...
            if (timing == EffectTiming.OnDestroyedAnyone)
            {
                ...
                {
                    return "[On Deletion] <De-Digivolve 1> 1 of your opponent's Digimon. (Trash 1 card from the top of one of your opponent's Digimon. If it has no digivolution cards, or becomes a level 3 Digimon, you can't trash any more cards.)";
                }
