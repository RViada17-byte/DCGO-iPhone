...
{
    public class BT15_031 : CEntity_Effect
    {
        public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
        {
            ...
            {
                return "Return 1 level 5 or lower Digimon to hand.";
            }

            string SharedEffectDescription(string tag)
            {
                return $"[{tag}] Return 1 of your opponent's level 5 or lower Digimon to the hand.";
            }
            ...
            if (timing == EffectTiming.OnEndTurn)
            {
                ...
                {
                    if (!cardSource.CardNames.Contains("MetalSeadramon"))
                    {
