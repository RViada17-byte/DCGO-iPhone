...
{
    public class EX2_074 : CEntity_Effect
    {
        public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
        {
            ...
            if (timing == EffectTiming.OnDiscardLibrary)
            {
                ...
                {
                    return "When this card is trashed from your deck, delete 1 of your opponent's level 4 or lower Digimon.";
                }
            ...
            {
                ActivateClass activateClass = new ActivateClass();
                activateClass.SetUpICardEffect("Delete oppponent's all Digimons with the highest level", CanUseCondition, card);
                activateClass.SetUpActivateClass(CanActivateCondition, ActivateCoroutine, -1, false, EffectDiscription());
                cardEffects.Add(activateClass);

                string EffectDiscription()
                {
                    return "[When Digivolving] Delete all of your opponent's Digimon with the highest level.";
                }
            ...
            if (timing == EffectTiming.None)
            {
                ...
                cardEffects.Add(CardEffectFactory.ChangeSelfSAttackStaticEffect(changeValue: count(), isInheritedEffect: false, card: card, condition: Condition));
            }
