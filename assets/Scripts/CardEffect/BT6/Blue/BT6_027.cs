...
using System;
using Photon.Pun;
public class BT6_027 : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        if (timing == EffectTiming.OnEnterFieldAnyone)
        {
            ...
            {
                return "[When Digivolving] Trash 1 digivolution card from the top of 1 of your opponent's Digimon.";
            }
        ...
        if (timing == EffectTiming.OnAllyAttack)
        {
            ...
            activateClass.SetHashString("Unsuspend_BT6_027");
            cardEffects.Add(activateClass);

            string EffectDiscription()
            {
                return "[When Attacking][Once Per Turn] If your opponent has no Digimon with digivolution cards in play, unsuspend this Digimon.";
            }
            ...
            bool CanActivateCondition(Hashtable hashtable)
            {
                if (CardEffectCommons.IsExistOnBattleArea(card))
                {
                    if (card.Owner.Enemy.GetBattleAreaDigimons().Count((permanent) => permanent.DigivolutionCards.Count >= 1) == 0)
                    {
                        return true;
                    }
                }
