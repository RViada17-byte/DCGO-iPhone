...
public class BT6_016 : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        if (timing == EffectTiming.OnAllyAttack)
        {
            ...
            activateClass.SetUpICardEffect("Play 1 Digimon with [Sistermon] in its name from hand or trash", CanUseCondition, card);
            activateClass.SetUpActivateClass(CanActivateCondition, ActivateCoroutine, -1, true, EffectDiscription());
            cardEffects.Add(activateClass);

            string EffectDiscription()
            {
                return "[When Attacking] You may play 1 Digimon card with [Sistermon] in its name from your hand or trash without paying its memory cost. ";
            }
        ...
        if (timing == EffectTiming.OnEnterFieldAnyone)
        {
            ...
            string EffectDiscription()
            {
                return "[Your Turn][Once Per Turn] When you play another Digimon, this Digimon gets +3000 DP and <Piercing> for the turn. (When this Digimon attacks and deletes an opponent's Digimon and survives the battle, it performs any security checks it normally would.)";
            }
