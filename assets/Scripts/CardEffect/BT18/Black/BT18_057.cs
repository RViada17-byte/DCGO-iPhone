...
{
    public class BT18_057 : CEntity_Effect
    {
        public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
        {
            ...
            if (timing == EffectTiming.BeforePayCost)
            {
                ...
                activateClass.SetHashString("ReduceDigivolutionCost-1_BT18_57");
                cardEffects.Add(activateClass);

                string EffectDescription()
                {
                    return
                        "[Your Turn] (Once Per Turn) When this Digimon or any of your Tamers would digivolve into a multicolored Digimon that is black or yellow, reduce the digivolution cost by 1.";
                }
                ...
                bool DigivolveToCardCondition(CardSource cardSource)
                {
                    return cardSource.IsDigimon && cardSource.CardColors.Count > 1 &&
                           (cardSource.CardColors.Contains(CardColor.Black) || cardSource.CardColors.Contains(CardColor.Yellow));
                }
