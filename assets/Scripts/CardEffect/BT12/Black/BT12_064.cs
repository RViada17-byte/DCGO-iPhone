...
{
    public class BT12_064 : CEntity_Effect
    {
        public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
        {
            ...
            if (timing == EffectTiming.OnEnterFieldAnyone)
            {
                ...
                {
                    return "[When Digivolving] <De-Digivolve 1> on 1 of your opponent's level 5 or lower Digimon. For every 2 cards in this Digimon's digivolution cards, add 1 to the max level of the Digimon you can choose with this effect.";
                }
                                ...
                                if (permanent.TopCard.Owner == card.Owner.Enemy)
                                {
                                    if (permanent.TopCard.Owner.GetBattleAreaPermanents().Contains(permanent))
                                    {
                                        if (permanent.TopCard.HasLevel)
                                        {
                                            ...
                                            {
                                                maxLevel += card.PermanentOfThisCard().DigivolutionCards.Count / 2;
                                            }
