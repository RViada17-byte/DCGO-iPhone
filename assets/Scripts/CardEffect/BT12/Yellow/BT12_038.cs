...
{
    public class BT12_038 : CEntity_Effect
    {
        public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
        {
            ...
            if (timing == EffectTiming.OnEnterFieldAnyone)
            {
                ...
                {
                    return "[When Digivolving] If you don't have a [Marcus Damon] in play, you may play 1 [Marcus Damon] from your hand without paying its cost.";
                }
                ...
                bool CanActivateCondition(Hashtable hashtable)
                {
                    if (CardEffectCommons.IsExistOnBattleArea(card))
                    {
                        ...
                        {
                            if (card.Owner.GetBattleAreaPermanents().Count((permanent) => permanent.TopCard.CardNames.Contains("Marcus Damon") || permanent.TopCard.CardNames.Contains("MarcusDamon")) == 0)
                            {
                                return true;
                            }
                        }
            ...
            if (timing == EffectTiming.OnTappedAnyone)
            {
                ...
                activateClass.SetHashString("DP-2000_BT12_038");
                cardEffects.Add(activateClass);
