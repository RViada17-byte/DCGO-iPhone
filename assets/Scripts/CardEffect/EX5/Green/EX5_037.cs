...
public class EX5_037 : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        if (timing == EffectTiming.OnUseOption)
        {
            ...
            activateClass.SetUpActivateClass(CanActivateCondition, ActivateCoroutine, 1, false, EffectDiscription());
            activateClass.SetHashString("Memory+1_EX5_037");
            cardEffects.Add(activateClass);

            string EffectDiscription()
            {
                return "[Your Turn] When you use an use Option card with a cost of 1 or more, gain 1 memory.";
            }
            ...
            bool CanUseCondition(Hashtable hashtable)
            {
                if (CardEffectCommons.IsExistOnBattleArea(card))
                {
                    ...
                    {
                        if (CardEffectCommons.CanTriggerWhenOwnerUseOption(hashtable, null, (cost) => cost >= 1, card))
                        {
                            return true;
                        }
                    }
