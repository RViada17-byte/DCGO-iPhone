...
{
    public class LM_029 : CEntity_Effect
    {
        public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
        {
            ...
            if (timing == EffectTiming.OptionSkill)
            {
                ...
                activateClass.SetUpICardEffect("Digivolve into a yellow Digimon from hand with reduced cost", CanUseCondition, card);
                activateClass.SetUpActivateClass(null, ActivateCoroutine, -1, false, EffectDescription());
                cardEffects.Add(activateClass);

                string EffectDescription()
                {
                    return "[Main] 1 of your yellow Digimon may digivolve into a yellow Digimon card in the hand with the digivolution cost reduced by 3. Then, place this card in the battle area.";
                }
                ...
                {
                    return cardSource.IsDigimon && cardSource.CardColors.Contains(CardColor.Yellow);
                }
                ...
                IEnumerator ActivateCoroutine(Hashtable hashtable)
                {
                    if (CardEffectCommons.HasMatchConditionPermanent(CanSelectOwnPermanentCondition))
                    {
                        ...
                        selectPermanentEffect.SetUpCustomMessage("Select 1 yellow Digimon that will digivolve.", "The opponent is selecting 1 Digimon that will digivolve.");
                    ...
                    if (cardSource != null)
                    {
                        if (cardSource.Owner == card.Owner)
                        {
                            if (CardEffectCommons.CanPlayAsNewPermanent(cardSource: cardSource, payCost: false, cardEffect: activateClass))
                            {
                                ...
                                {
                                    if (cardSource.CardColors.Contains(CardColor.Yellow) && cardSource.CardDP <= 2000)
                                    {
                                        return true;
                                    }
                                }
                        ...
                        if (card.Owner.GetBattleAreaDigimons().Count == 0)
                        {
                            if (CardEffectCommons.HasMatchConditionOwnersCardInTrash(card, (cardSource) => CanSelectCardCondition1(cardSource)))
                            {
                                ...
                                selectCardEffect.SetUp(
                                            ...
                                            message: "Select 1 yellow Digimon with 2000 DP or less to play.",
                                            maxCount: maxCount,
                                            canEndNotMax: false,
                                            isShowOpponent: true,
                                            mode: SelectCardEffect.Mode.Custom,
                                ...
                                selectCardEffect.SetUpCustomMessage("Select 1 yellow Digimon with 2000 DP or less to play.", "The opponent is selecting 1 card to play.");
            ...
            {
                ActivateClass activateClass = new ActivateClass();
                activateClass.SetUpICardEffect($"Play a yellow Digimon from trash.", CanUseCondition, card);
                ...
                string EffectDiscription()
                {
                    return "[Security] You may play 1 yellow Digimon card with 2000 DP or less from your trash without paying the cost. Then, add this card to the hand.";
                }
