...
{
    public class LM_027 : CEntity_Effect
    {
        public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
        {
            ...
            if (timing == EffectTiming.OptionSkill)
            {
                ...
                activateClass.SetUpICardEffect("Digivolve into a red Digimon from hand with reduced cost", CanUseCondition, card);
                activateClass.SetUpActivateClass(null, ActivateCoroutine, -1, false, EffectDescription());
                cardEffects.Add(activateClass);

                string EffectDescription()
                {
                    return "[Main] 1 of your red Digimon may digivolve into a red Digimon card in the hand with the digivolution cost reduced by 3. Then, place this card in the battle area.";
                }
                ...
                {
                    return cardSource.IsDigimon && cardSource.CardColors.Contains(CardColor.Red);
                }
                ...
                IEnumerator ActivateCoroutine(Hashtable hashtable)
                {
                    if (CardEffectCommons.HasMatchConditionPermanent(CanSelectOwnPermanentCondition))
                    {
                        ...
                        selectPermanentEffect.SetUpCustomMessage("Select 1 red Digimon that will digivolve.", "The opponent is selecting 1 Digimon that will digivolve.");
                    ...
                    if (cardSource != null)
                    {
                        if (cardSource.Owner == card.Owner)
                        {
                            if (CardEffectCommons.CanPlayAsNewPermanent(cardSource: cardSource, payCost: false, cardEffect: activateClass))
                            {
                                ...
                                {
                                    if(cardSource.CardColors.Contains(CardColor.Red) && cardSource.CardDP <= 2000)
                                    {
                                        return true;
                                    }                               
                                }
            ...
            {
                ActivateClass activateClass = new ActivateClass();
                activateClass.SetUpICardEffect($"Play a red Digimon from trash.", CanUseCondition, card);
                ...
                string EffectDiscription()
                {
                    return "[Security] You may play 1 red Digimon card with 2000 DP or less from your trash without paying the cost. Then, add this card to the hand.";
                }
