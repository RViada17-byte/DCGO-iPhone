...
{
    public class BT16_079 : CEntity_Effect
    {
        public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
        {
            ...
            if (timing == EffectTiming.None)
            {
                ...
                {
                    return targetPermanent.TopCard.CardNames.Contains("Cherubimon");
                }
            ...
            if (timing == EffectTiming.OnEnterFieldAnyone)
            {
                ...
                string EffectDiscription()
                {
                    return "[When Digivolving] [Once Per Turn] You may play 1 level 4 or lower yellow or green Digimon from your hand or trash without paying the cost.";
                }
            ...
            if (timing == EffectTiming.OnEndTurn)
            {
                ...
                activateClass.SetUpActivateClass(CanActivateCondition, ActivateCoroutine, 1, true, EffectDiscription());
                activateClass.SetHashString("DeleteDigimon_BT16_079");
                cardEffects.Add(activateClass);

                string EffectDiscription()
                {
                    return "[End of Your Turn] [Once Per Turn] If [Cherubimon] or [X-Antibody] is in this Digimon's digivolution cards, delete 1 of your opponent's level 4 or lower Digimon. For each of your other Digimon, add 1 to the maximum level this effect can choose.";
                }
                ...
                bool CanActivateCondition(Hashtable hashtable)
                {
                    if (CardEffectCommons.IsExistOnBattleArea(card))
                    {
                        if (card.PermanentOfThisCard().DigivolutionCards.Count((cardSource) => cardSource.CardNames.Contains("Cherubimon") || cardSource.CardNames.Contains("X Antibody") || cardSource.CardNames.Contains("XAntibody")) >= 1)
                        {
