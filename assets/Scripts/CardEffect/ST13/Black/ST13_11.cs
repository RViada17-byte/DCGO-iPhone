...
public class ST13_11 : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        if (timing == EffectTiming.OnEnterFieldAnyone)
        {
            ...
            activateClass.SetUpICardEffect("Place this Digimon under your Digimon's digivolution cards and your 1 Digimon gains Reboot", CanUseCondition, card);
            activateClass.SetUpActivateClass(CanActivateCondition, ActivateCoroutine, -1, false, EffectDiscription());
            cardEffects.Add(activateClass);

            string EffectDiscription()
            {
                return "[On Play] By placing this Digimon under 1 of your other Digimon that's red or has [Legend-Arms] in its traits as its bottom digivolution card, 1 of your Digimon gains <Reboot> until the end of your opponent's turn. (Unsuspend this Digimon during your opponent's unsuspend phase.)";
            }
            ...
            IEnumerator ActivateCoroutine(Hashtable _hashtable)
            {
                if (CardEffectCommons.HasMatchConditionPermanent(CanSelectPermanentCondition))
                {
                    ...
                    if (added)
                    {
                        ...
                        selectPermanentEffect.SetUpCustomMessage("Select 1 Digimon that will get Reboot.", "The opponent is selecting 1 Digimon that will get Reboot.");
        ...
        if (timing == EffectTiming.None)
        {
            bool Condition()
            {
                if (CardEffectCommons.IsExistOnBattleArea(card))
                {
                    ...
                    {
                        if (card.Owner.GetBattleAreaDigimons().Count(permanent => permanent.TopCard.CardTraits.Contains("Legend-Arms") || permanent.TopCard.CardColors.Contains(CardColor.Red)) >= 1)
                        {
                            return true;
                        }
                    }
