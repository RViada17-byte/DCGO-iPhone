...
using System;
using Photon.Pun;
public class BT7_023 : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        if (timing == EffectTiming.OnEnterFieldAnyone)
        {
            ...
            {
                return "[When Digivolving] If a card with [Hybrid] in its traits or [Tommy Himi] is in this Digimon's digivolution cards, 1 of your opponent's Digimon with no digivolution cards can't attack or block until the end of their next turn.";
            }
            ...
            bool CanActivateCondition(Hashtable hashtable)
            {
                ...
                {
                    if (card.PermanentOfThisCard().DigivolutionCards.Count((cardSource) => cardSource.CardTraits.Contains("Hybrid") || cardSource.CardNames.Contains("Tommy Himi") || cardSource.CardNames.Contains("TommyHimi")) >= 1)
                    {
