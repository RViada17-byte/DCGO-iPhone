...
//Unique Emblem: Primal Impact
namespace DCGO.CardEffects.P
{
    public class P_227 : CEntity_Effect
    {
        public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
        {
            ...
            if (timing == EffectTiming.OptionSkill)
            {
                ...
                activateClass.SetUpICardEffect("Reveal 3, add 1 [Tyrannomon] in name or [Reptile]/[Dinosaur] trait and 1 [Liberator] trait, bot deck the rest, place in battle area.", CanUseCondition, card);
                activateClass.SetUpActivateClass(null, ActivateCoroutine, -1, false, EffectDiscription());
                cardEffects.Add(activateClass);

                string EffectDiscription()
                {
                    return "[Main] Reveal the top 3 cards of your deck. Add 1 Digimon card with [Tyrannomon] in its name or the [Reptile] or [Dinosaur] trait and 1 [LIBERATOR] trait card among them to the hand. Return the rest to the bottom of the deck. Then, place this card in the battle area.";
                }
                ...
                IEnumerator ActivateCoroutine(Hashtable hashtable)
                {
                    yield return ContinuousController.instance.StartCoroutine(CardEffectCommons.SimplifiedRevealDeckTopCardsAndSelect(
                        ...
                        new SimplifiedSelectCardConditionClass(
                            ...
                            message: "Select 1 card with [Tyrannomon] in its name or the [Reptile]/[Dinosaur] trait.",
            ...
            if (timing == EffectTiming.OnEnterFieldAnyone)
            {
                ...
                bool PlayedPermanentCondition(Permanent permanent)
                {
                    return CardEffectCommons.IsPermanentExistsOnOwnerBattleArea(permanent, card)
                        && permanent.TopCard.EqualsCardName("Ryutaro Williams");
                }
            ...
            if (timing == EffectTiming.SecuritySkill)
            {
                CardEffectCommons.AddActivateMainOptionSecurityEffect(
                    ...
                    effectName: "Reveal 3, add 1 [Tyrannomon] in name or [Reptile]/[Dinosaur] trait and 1 [Liberator] trait, bot deck the rest, place in battle area.");
            }
