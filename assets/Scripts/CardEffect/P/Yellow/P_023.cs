...
public class P_023 : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        if (timing == EffectTiming.OptionSkill)
        {
            ...
            {
                return "[Main] If you have [T.K. Takaishi] in play, place 1 of your [Patamon] at the bottom of your security stack face down. Trash that Digimon's digivolution cards.";
            }
            ...
            bool CanSelectPermanentCondition(Permanent permanent)
            {
                ...
                {
                    if (permanent.TopCard.CardNames.Contains("T.K. Takaishi"))
                    {
                        return true;
                    }
            ...
            bool CanSelectPermanentCondition1(Permanent permanent)
            {
                if (card.Owner.CanAddSecurity(activateClass))
                {
                    if (CardEffectCommons.IsPermanentExistsOnOwnerBattleArea(permanent, card))
                    {
                        if (permanent.TopCard.CardNames.Contains("Patamon"))
                        {
                            return true;
                        }
                    }
            ...
            IEnumerator ActivateCoroutine(Hashtable _hashtable)
            {
                if (card.Owner.GetBattleAreaPermanents().Count(CanSelectPermanentCondition) >= 1)
                {
                    if (card.Owner.GetBattleAreaPermanents().Count(CanSelectPermanentCondition1) >= 1)
                    {
                        ...
                        selectPermanentEffect.SetUpCustomMessage("Select 1 [Patamon] to place to security.", "The opponent is selecting 1 [Patamon] to place to security.");
