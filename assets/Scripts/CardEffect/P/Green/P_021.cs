...
using System;
using Photon.Pun;
public class P_021 : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        if (timing == EffectTiming.OptionSkill)
        {
            ...
            {
                return "[Main] If you have [Mimi Tachikawa] in play, you may play a [Palmon] from your hand without paying its memory cost to return 1 of your [Mimi Tachikawa] cards to its owner's hand.";
            }
            ...
            bool CanSelectPermanentCondition(Permanent permanent)
            {
                ...
                {
                    if (permanent.TopCard.CardNames.Contains("Mimi Tachikawa"))
                    {
                        return true;
                    }

                    if (permanent.TopCard.CardNames.Contains("MimiTachikawa"))
                    {
                        return true;
                    }
                }
                ...
                if (card.Owner.GetBattleAreaPermanents().Count(CanSelectPermanentCondition) >= 1)
                {
                    ...
                    if (card.Owner.HandCards.Count(CanSelectCardCondition) >= 1)
                    {
                        ...
                        if (selectedCards.Count >= 1)
                        {
                            ...
                            {
                                if (CardEffectCommons.CanPlayAsNewPermanent(cardSource: selectedCard, payCost: false, cardEffect: activateClass, root: SelectCardEffect.Root.Hand))
                                {
