...
{
    public class P_171 : CEntity_Effect
    {
        public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
        {
                                ...
                                if (permanent.TopCard != null)
                                {
                                    if (permanent.TopCard.Owner == card.Owner)
                                    {
                                        if (permanent.IsDigimon)
                                        {
                                            ...
                                            {
                                                if (permanent.TopCard.CardColors.Contains(CardColor.Blue) || permanent.TopCard.CardColors.Contains(CardColor.Black))
                                                {
            ...
            if (timing == EffectTiming.BeforePayCost)
            {
                ...
                activateClass.SetUpICardEffect("If [Deep Savers] is in your face up security cards get Play Cost -4", CanUseCondition, card);
                activateClass.SetUpActivateClass(CanActivateCondition, ActivateCoroutine, -1, false, EffectDiscription());
                activateClass.SetHashString("PlayCost-4_P_171");
                cardEffects.Add(activateClass);

                string EffectDiscription()
                {
                    return "When this card would be played, if [Deep Savers] is in your face up security cards, reduce the play cost by 4.";
                }
            ...
            if (timing == EffectTiming.None)
            {
                ...
                bool CanUseCondition(Hashtable hashtable)
                {
                    ...
                    {
                        ICardEffect activateClass = card.EffectList(EffectTiming.BeforePayCost).Find(cardEffect => cardEffect.EffectName == "If [Deep Savers] is in your face up security cards get Play Cost -4");
            ...
            if (timing == EffectTiming.OnEnterFieldAnyone)
            {
                ...
                string EffectDiscription()
                {
                    return "[On Play] Trash the top 2 digivolution cards of all of your opponent's Digimon. Then, delete 1 of their Digimon with no digivolution cards.";
                }

                bool CanDigionWithoutSourcesCondition(Permanent permanent)
                {
                ...
                IEnumerator ActivateCoroutine(Hashtable _hashtable)
                {
                    ...
                    {
                        int maxCount = Math.Min(1, CardEffectCommons.MatchConditionPermanentCount(CanDigionWithoutSourcesCondition));

                        SelectPermanentEffect selectPermanentEffect = GManager.instance.GetComponent<SelectPermanentEffect>();

                        selectPermanentEffect.SetUp(
                            selectPlayer: card.Owner,
                            canTargetCondition: CanDigionWithoutSourcesCondition,
            ...
            if (timing == EffectTiming.OnEnterFieldAnyone)
            {
                ...
                string EffectDiscription()
                {
                    return "[When Digivolving] Trash the top 2 digivolution cards of all of your opponent's Digimon. Then, delete 1 of their Digimon with no digivolution cards.";
                }
