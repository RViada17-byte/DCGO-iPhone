...
public class BT3_107 : CEntity_Effect
{
            ...
            IEnumerator ActivateCoroutine(Hashtable _hashtable)
            {
                if (CardEffectCommons.HasMatchConditionPermanent(CanSelectPermanentCondition))
                {
                    ...
                    IEnumerator SelectPermanentCoroutine(Permanent permanent)
                    {
                        ...
                        {
                            if (selectedPermanent.TopCard.GetCostItself <= 4)
                            {
                                if (selectedPermanent.TopCard.HasPlayCost)
                                {
                                    yield return ContinuousController.instance.StartCoroutine(new DestroyPermanentsClass(new List<Permanent>() { selectedPermanent }, CardEffectCommons.CardEffectHashtable(activateClass)).Destroy());
                                }
                            }
                        }
                    }
